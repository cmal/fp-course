...
findRight p (ListZipper l x r) =
  ...
  case break p r of
    (_, Nil) -> IsNotZ
    (a, b) -> IsZ $ ListZipper l1 x1 r1
                    where l1 = reverse a ++ x:.l
                          (x1:.r1) = b
...
moveLeftLoop (ListZipper l x r) =
  ...
  case l of
    Nil -> ListZipper l1 x1 Nil where (x1:.l1) = reverse (x:.r)
    (x2:.l2) -> ListZipper l2 x2 (x:.r)
...
moveRightLoop (ListZipper l x r) =
  ...
  case r of
    Nil -> ListZipper Nil x1 r1 where (x1:.r1) = reverse $ x:.l
    (x2:.r2) -> ListZipper (x:.l) x2 r2
...
moveLeft (ListZipper l x r) =
  ...
  case l of
    Nil -> IsNotZ
    (x2:.l2) -> IsZ $ ListZipper l2 x2 (x:.r)
...
moveRight (ListZipper l x r) =
  ...
  case r of
    Nil -> IsNotZ
    (x2:.r2) -> IsZ $ ListZipper (x:.l) x2 r2
...
swapLeft (ListZipper l x r) =
  ...
  case l of
    Nil -> IsNotZ
    (x2:.l2) -> IsZ $ ListZipper (x:.l2) x2 r
...
swapRight (ListZipper l x r) =
  ...
  case r of
    Nil -> IsNotZ
    (x2:.r2) -> IsZ $ ListZipper l x2 (x:.r2)
...
moveLeftN n z
  | n == 0 = IsZ z
  | n < 0 = moveRight -<< moveLeftN (n + 1) z
  | otherwise = moveLeft -<< moveLeftN (n - 1) z
...
moveRightN n z
  | n == 0 = IsZ z
  | n < 0 = moveLeft -<< moveRightN (n + 1) z
  | otherwise = moveRight -<< moveRightN (n - 1) z
...
moveLeftN' n z =
  case moveLeftN n z of
    (IsZ y) -> Right y
    _ -> Left $ length $ (if n > 0 then lefts else rights) z
...
end z = ListZipper l1 x1 Nil where (x1:.l1) = reverse $ toList z
...
start z = ListZipper Nil x1 r1 where (x1:.r1) = toList z
...
deletePullLeft (ListZipper (y:.l) _ r) = IsZ $ ListZipper l y r
...
deletePullRight (ListZipper l _ (y:.r)) = IsZ $ ListZipper l y r
...
instance Extend ListZipper where
  (<<=) f z =
    -- error "todo: Course.ListZipper (<<=)#instance ListZipper"
    ListZipper (unfoldr (\z_ -> (\x -> (f x, x)) <$> toOptional (moveLeft z_)) z)
               (f z)
               (unfoldr (\z_ -> (\x -> (f x, x)) <$> toOptional (moveRight z_)) z)
...
instance Extend MaybeListZipper where
  (<<=) _ IsNotZ = IsNotZ
  (<<=) f (IsZ z) = IsZ $ ListZipper (unfoldr (\z_ -> (\x -> (f (IsZ x), x)) <$> toOptional (moveLeft z_)) z)
                                     (f (IsZ z))
                                     (unfoldr (\z_ -> (\x -> (f (IsZ x), x)) <$> toOptional (moveRight z_)) z)
    -- error "todo: Course.ListZipper (<<=)#instance MaybeListZipper"
