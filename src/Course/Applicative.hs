...
lift4 ::
  ...
  -> f d
  -> f e
lift4 f4 fa fb fc fd = f4 <$> fa <*> fb <*> fc <*> fd
...
replicateA ::
  ...
  -> f a
  -> f (List a)
-- https://github.com/svanderbleek/fp-course-yolo/blob/master/src/Course/Applicative.hs
replicateA n a = sequence $ replicate n a
-- replicate :: (Ord n, Num n) => n -> a -> List a
...
-- from https://github.com/svanderbleek/fp-course-yolo/blob/master/src/Course/Applicative.hs
filtering p = foldRight reduceEffects (pure Nil)
              where reduceEffects a = lift2 (consOrSkip a) (p a)
