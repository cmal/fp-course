...
(|||) px py =
  ...
  P $ \input -> let resultX = parse px input
                in if isErrorResult resultX
                   then (parse py input)
                   else resultX
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser a
    -> Parser b
  (=<<) apb pa = P $ \input -> onResult (parse pa input) (\input1 a -> (parse (apb a) input1))
...
list1 ::
  Parser a
  -> Parser (List a)
-- list1 p = (\a -> P $ \input -> (a :.) <$> parse (list p) input) =<< p
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy pred = P $ \input -> case input of
                               Nil -> UnexpectedEof
                               (x:.xs) -> if pred x
                                             then Result xs x
...
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser Nil = pure Nil -- what's this mean?
sequenceParser (p:.ps) = (\a -> pure (a :.) <*> sequenceParser ps) =<< p
...
firstNameParser ::
  Parser Chars
firstNameParser = (\u -> pure (u :.) <*> (list lower)) =<< upper
...
surnameParser ::
  Parser Chars
surnameParser = (\cs -> pure (cs ++) <*> (list lower)) =<< (\u -> pure (u:.) <*> (thisMany 5 lower)) =<< upper
