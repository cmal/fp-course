...
string ::
  Chars
  -> Parser Chars
string chars = sequenceParser (is <$> chars)
...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok o c a = between (charTok o) (charTok c) a
...
hex ::
  Parser Char
hex = (thisMany 4 (satisfy isHexDigit)) >>= (\d4 -> case readHex d4 of
...
sepby1 ::
  ...
  -> Parser s
  -> Parser (List a)
sepby1 pa ps = pa >>= (\a -> pure (a:.) <*> (list $ ps *> pa))
